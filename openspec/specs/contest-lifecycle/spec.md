...
Manages the lifecycle of a CTF contest including starting, stopping, and automatic tick/round advancement based on configuration.
...
### Requirement: Contest auto-starts based on schedule
The system SHALL automatically start the contest when `is_running` is `true` and the `start_date` has passed.

#### Scenario: Auto-start when date has passed
- **WHEN** server starts and `is_running` is `true` and `start_date` is in the past
- **THEN** system starts the contest immediately

#### Scenario: Schedule start for future date
- **WHEN** server starts and `is_running` is `true` and `start_date` is in the future
- **THEN** system schedules contest to start at exactly `start_date`

#### Scenario: Reload schedule when config changes
- **WHEN** `start_date` or `is_running` config is updated while server is running
- **THEN** system cancels any existing scheduled start and re-evaluates based on new values
